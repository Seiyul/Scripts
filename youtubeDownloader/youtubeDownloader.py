...
zip_ffmpeg = 'ffmpeg.zip'
zip_ytdlp = 'yt-dlp.zip'
...
def extraer_en_raiz(zip_path, destino='.'):
    # Abrir el archivo .zip
    with zipfile.ZipFile(zip_path, 'r') as zip_ref:
        for member in zip_ref.infolist():
            # Obtener solo el nombre del archivo (sin carpetas)
            nombre_archivo = os.path.basename(member.filename)
            if nombre_archivo:  # Verificar que no sea una carpeta
                source = zip_ref.open(member)
                # Guardar el archivo en la raíz (destino)
                target = open(os.path.join(destino, nombre_archivo), "wb")
                with source, target:
...
def get_yt_dlp_formats(url):
    ...
    columns = [
        ...
        {"COLUMN": "FILESIZE", "START": 34, "END": 44},
...
def download_video_and_audio(url, formats, video_title):
    video_formats = [f for f in formats if f['FILESIZE'] and f['ACODEC'] == 'video only' and 'Premium' not in f['MORE INFO']]
...
extraer_en_raiz(zip_ffmpeg)
extraer_en_raiz(zip_ytdlp)
